...
import { collectDocuments, parseDocument, walkElements, type SourceDocument } from '@/services/conversion/model'
...
export function DocumentReader(props: ComponentProps<typeof MarkdownPreview>) {
  ...
  const needsLocalAssets=useMemo(()=>{
    if(!current?.node.standalone)return false
    let needed=false
    walkElements(parseDocument(props.content,'asset-check').tree,node=>{
      const src=node.properties.src
      if(['img','video','audio','source'].includes(node.tagName)&&typeof src==='string'&&src&&!isRemoteUrl(src))needed=true
    })
    return needed
  },[props.content,current?.node.standalone])
  useEffect(() => {
